...
4. 设 $ (X_1,\mathscr{T}_1) $ 和 $ (X_2,\mathscr{T}_2) $ 为拓扑空间, $ X=X_1\times X_2 $ ,定义 $ X $ 的拓扑为 $  $ \mathscr{T}:=\{O\subset X|O\text{可以表示为形如}O_1\times O_2\text{的子集之并},O_1 \in\mathscr{T}_1,O_2\in \mathscr{T}_2\} $  $ 
则 $ \mathscr{T} $ 称为 $ X $ 的**乘积拓扑(product topology)**.
...
> 
![乘积拓扑](/assets/img/in-post/2024/2024-05-27/诱导拓扑.jpg)
